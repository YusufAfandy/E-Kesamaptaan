...
<div class="max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tighter">Entry Nilai Jasmani</h2>
            <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest italic">Bagian SDM — Standard Operasional Samapta A & B</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-12">
            <form action="{{ url('/samapta/store') }}" method="POST">
                {{ csrf_field() }}
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="user_id" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-900 outline-none transition">
                                ...
                                {{-- Kita ambil langsung data personil di sini --}}
                                @foreach(\App\User::where('role', 'personil')->get() as $p)
                        ...
                        <div>
                            ...
                            <input type="text" name="periode" placeholder="Contoh: 2024-S1" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-900 transition">
                        </div>
                        ...
                        <div class="pt-6 border-t border-slate-50">
                            ...
                            <input type="number" name="lari_meter" placeholder="Contoh: 2400" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-900 transition">
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="pull_up" required class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold text-slate-700">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Sit Up</label>
                                <input type="number" name="sit_up" required class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold text-slate-700">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Push Up</label>
                                <input type="number" name="push_up" required class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold text-slate-700">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Shuttle Run (Detik)</label>
                                <input type="number" step="0.01" name="shuttle_run" required class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold text-slate-700">
                            </div>
                        </div>

                        <!-- KOTAK SCORING ENGINE -->
                        <div class="bg-yellow-500 rounded-[2.5rem] p-8 text-[#0F172A] relative overflow-hidden shadow-xl shadow-yellow-500/20 mt-6">
                            ...
                            <h4 class="text-3xl font-black italic uppercase tracking-tighter leading-none">Hitung Otomatis</h4>
                            <p class="text-[9px] font-bold uppercase mt-3 opacity-60">Nilai akhir akan dikalkulasi oleh sistem sesuai klasifikasi umur dan jenis kelamin.</p>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 absolute right-[-10px] bottom-[-10px] opacity-10" fill="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-14 pt-8 border-t border-slate-50">
                    <button type="submit" class="w-full bg-[#0F172A] hover:bg-black text-white font-black py-5 rounded-[1.5rem] shadow-2xl transition-all active:scale-[0.98] text-[10px] uppercase tracking-[0.2em]">
                        Finalisasi & Simpan Nilai Jasmani
                    </button>
                </div>
            </form>
        </div>
